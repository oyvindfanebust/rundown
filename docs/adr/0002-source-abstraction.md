...
Linear, Claude Code logs, …) to help plan. A Source is the pluggable, read-only component for each
...
The central trust rule (CLAUDE.md / AGENTS.md) constrains every decision: untrusted
calendar/mail/source content meets a model only in the sandboxed, tool-less summarizer, and
tool-capable code never reasons over raw source content. The abstraction's ultimate consumer is
therefore the Summarizer, a text model, while the Aggregator, which is tool-capable code, only
groups, orders, and attributes items and never interprets their content.
...
A Source is a read-only adapter for one backend system, which is also one auth boundary. Graph is
a single source (not calendar plus mail as two), because auth is inherently per-backend: one Graph
...
The governing principle for whether a method is required or optional is this: require a method when
every source has a meaningful total answer, because the concept is universal; keep it optional when
the capability genuinely varies and its absence is the type-checked signal. Readiness is universal,
since every source can answer "can I read you right now?" (a local source answers "yes, always"),
so `status()` is required. Interactive login is a capability that varies; forcing it to be total
would need a throwing stub or a redundant capability flag, both worse than letting the method's
presence be the typed declaration, so `login?()` stays optional.

`SourceStatus` is a discriminated union rather than a boolean pair, so nonsensical combinations are
...
There is one read operation, a time-windowed read, with no separate `state()` operation. The
...
issues) may return standing or open items whose last activity predates the window, because they
...
The core carries only what the aggregator touches to group, order, and attribute; everything
else lives in `extras`, which the source shapes into a readable sub-object for the summarizer.
Deliberately not in the core:

- People and participants — organizer, attendees, from/to, assignee, author. Almost as universal
...
- **Trust split.** The structural-trusted field set is exactly `{source, kind, timestamp,
  end}`, produced and constrained by rundown's own source module, and safe to template into an
  agent-facing context. Everything else, including `id` and `url`, is untrusted backend
  content, since a hostile backend controls its bytes. Only the summarizer may reason over untrusted
  content; the aggregator and planner treat it as opaque strings. This ADR fixes the contract; the
...
- **Config.** Each source declares its credential and option needs so `status()` can answer
  ...
  taking a per-call `options` argument — a source that reads config needs it in both methods, and a
  `status(options?)` parameter would be a wart on the one method that has no natural place for it.
...
- **Timezone.** Not a `read` parameter. Applied when constructing the window ("this week in
  Oslo" → two instants) and when displaying the brief, both of which are config concerns. A source
  needing a display tz (e.g. Graph all-day events) reads it from its own config options. This lifts
  today's hardcoded `TIMEZONE = "Europe/Oslo"` out of Sources.
...
(matching ADR-0001's sealed-binary model; a new source means a new module plus a recompile and
release). The `read` contract is deliberately serialization-friendly: `window` and items are plain
serializable values, identical to what would cross a process boundary, so out-of-tree and
...
- **No agent-facing per-source subcommand.** Raw `read` output stays internal to `rundown brief`
  ...
  at the level that matters: `rundown` is the CLI the skill drives, and sources are internal modules
  of it.
...
- One uniform item shape lets the Aggregator treat calendar events, emails, chat messages,
  issues, and sessions identically: group, order, and attribute by four trusted structural fields.
- The trust split gives [ADR-0004](0004-trust-boundary-enforcement.md) a precise foundation:
  exactly four fields are safe to surface structurally; everything else is opaque and untrusted.
- The `extras` bag absorbs source variety without schema churn, so a fifth source (Claude Code
...
- In-process now, with a process-shaped contract, gives a clean, pre-planned evolution to plugins
  later.

**Negative / accepted costs**
- A new source requires a recompile plus release (no drop-in third-party sources in v1), accepted;
