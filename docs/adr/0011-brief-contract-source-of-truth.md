...
to collapse three hand-synced representations into one.

## Context

The Brief's output contract is the `{summary, items}` pair the Summarizer emits, its `ExtractedItem`
shape, and the fixed `kind` enum ([ADR-0005](0005-planning-layer.md) §2–4). Until now it was spelled
out three times, by hand, in two files:

1. TypeScript types in `src/domain.ts` (`ExtractedKind`, `Evidence`, `ExtractedItem`,
   `SummarizerOutput`), which the code compiles against.
2. A hand-written JSON Schema literal (`BRIEF_OUTPUT_SCHEMA`) in `src/plan.ts`, which the
...
3. Prose in `plan.ts`'s `ITEM_RULES`, listing each `kind` with its one-line meaning, which the model
   reads to classify items.
...
place left the other two silently stale. This is a correctness hazard. A JSON Schema that drifts
from the TS type produces model output the code mistypes, and prose that drifts from the enum
mis-instructs the model, yet the drift is invisible until it bites at runtime.

## Decision

### 1. `src/brief-contract.ts` is the single source of truth, defined in Zod v4

One new module owns the contract and imports only `zod`. From one Zod schema it derives all three
representations, so they cannot diverge:

- The JSON Schema is generated rather than hand-written: `BRIEF_OUTPUT_SCHEMA = z.toJSONSchema(
  SummarizerOutputSchema, { reused: "inline" })`, post-processed to strip the generator's top-level
  `$schema` key. `z.strictObject(…)` makes every object node carry `additionalProperties: false`,
  which the structured-output API requires, and `reused: "inline"` keeps the schema flat, with no
  `$defs`/`$ref`, since `Evidence`/`ExtractedItem` are each used once. The generated object is
...
- The TypeScript types are inferred: `Evidence`, `ExtractedItem`, `SummarizerOutput` are `z.infer<…>`
  ...
  optional.
- The prose is composed from the contract: `KIND_DESCRIPTIONS: Record<ExtractedKind, string>` holds
  the one-line meaning of each `kind`, and the Planner maps over it to build `ITEM_RULES`'
  classification bullets. `Record<ExtractedKind, …>` makes exhaustiveness a typecheck, so adding a
...
and the Summarizer still has zero tools. This is a source-organization change, not a security
change.
...
- The contract is small and central: a two-object schema on the pipeline's output contract, edited
  rarely but wrong silently when it drifts. That drift is exactly the class of bug a single source
  of truth eliminates.
- The codebase is type-heavy and relies on the typecheck as a hard gate, since the `Untrusted<T>`
  sole-unwrap guarantee is a dev-time typecheck. Zod's infer-from-schema fits that approach: the
  types stay first-class while the schema becomes generated rather than transcribed.
...
- The generated schema's key order differs from the old hand-written literal. This is semantically
  irrelevant for JSON Schema but makes for a larger textual diff on first introduction.
