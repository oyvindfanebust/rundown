...
The central trust rule (CLAUDE.md / AGENTS.md) governs every decision: untrusted source content
...
resolution from the user's timezone and reporting period) applied to every selected source. Sources
are pulled concurrently, since they are independent I/O and one slow backend must not serialize the
rest. The single window is what makes cross-source bucketing coherent: one `from`/`to`/`now`
yardstick means "standing vs. recent vs. upcoming" is defined identically for a calendar event and a
Jira issue.
...
caller) decides which sources run and with what options, and the Aggregator just pulls. The
...
The Aggregator only pulls the named sources, merges, buckets, and sorts. It reads no config
and makes no policy choice (mirroring how ADR-0002 lifted timezone out of Sources). Source-level
...
format choice, which is interpretation it must not do. The consumer is a text model reading rendered
text, and a flat list plus a `bucket` tag carries everything needed to render any grouping at
template time.
...
`bucket` is a derived, structural-trusted label (a pure function of trusted fields), not
...
- Uses only trusted structural fields, so sorting never touches untrusted content.
- Fully deterministic: the same inputs produce a byte-identical bundle. The downstream consumer is
  an LLM, so stable ordering aids prompt caching and makes runs reproducible; the tiebreak removes
...
The bundle order is a deterministic default and tiebreak, not the final reading order. Standing
...
records provenance and counts, never a per-source status field; error and unauth cases live in the
...
The aggregator emits the union of all items, with no deduplication and no cross-source correlation.
This is forced, not deferred:

- **Correlation is content-based, and content is untrusted.** Matching a Linear issue to a Slack
  mention requires parsing the message body/url, which is untrusted `extras`. The aggregator is
  forbidden to interpret untrusted content, so it structurally cannot correlate without breaking the
  rule.
...
Even same-source `{source, id}` collisions are left alone, because dropping a "duplicate" is itself
an interpretation, and the cost of a stray dupe is negligible. The Aggregator stays simple.
...
- **The agent receives only the summarizer's output** — a safe brief, potentially with extracted
  intent/tasks — never the raw bundle. The summarizer's output is where untrusted content becomes
  safe to surface.
...
**Positive**
- A flat annotated list keeps the Aggregator to pull, merge, bucket, and sort, and
  defers all format choices to the Planner. The Aggregator never interprets content, satisfying
  the central trust rule.
